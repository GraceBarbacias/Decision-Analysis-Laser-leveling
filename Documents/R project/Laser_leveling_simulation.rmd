...
1.  Improved Water Management: Laser leveling technology ensures precise
    ...
    enables uniform distribution across the rice fields **(Nguyen &
    Thang, 2019)**. By achieving a consistent water depth, laser
    leveling reduces water wastage, enhances irrigation efficiency, and
    minimizes the risk of both waterlogging and drought stress **(Ngoc
    et al., 2012)**.
...
2.  Increased Productivity and Yield: Uneven land surfaces can hinder
    ...
    ensures a balanced distribution of sunlight **(Tuan et al., 2014)**.
...
3.  Cost and Labor Savings: Traditional manual leveling methods are
    ...
    reducing the need for manual labor **(Nguyen et al., 2021)**. This
...
4.  Environmental Sustainability: Laser leveling helps mitigate negative
    ...
    **(Ngoc et al., 2012)**. Additionally, by promoting uniform crop
...
we decided to focus on the Economic Criteria and Ecological Criteria of
laser leveling, as recommended by **(Oertlé et al., 2019)** where they
explain the importance of introducing the topic, since it is currently
not that known in Vietnam. They remark that is also important to
demonstrate that the technology (laser leveling in this case) could have
an economic an environmental potential for cooperative farmers. We know
that we can implement this technology in Vietnam because the goverment
has runned a program of "5 for 1 program" in Vietnam, to promote
sustainable rice production, which includes laser leveling as an
strategy. The following steps after this project will be to to hold a
...
contains all the input variables and each basic values used in the
model.
...
LL_model_function <- function(x, varnames){
  ...
  # Precalculationn for the change in land use with the intervention of Laser Leveling
   precalc_LL_land_use <-
    ...
    vv(LL_land_area_ha, var_CV, n_years) 
